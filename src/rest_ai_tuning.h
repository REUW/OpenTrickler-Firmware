...
/**
 * POST /rest/ai_machine_calibration_start
 *
 * Start a saved-profile machine calibration session. This measures scale lag,
 * post-stop tail, and throw uncertainty using the existing powder model.
 */
...
/**
 * GET /rest/ai_kernel_weight
 * Set (weight_gn > 0) or clear (weight_gn omitted/0) a user-entered kernel
 * weight override for profile_idx. Returns the resulting effective kernel
...
/**
 * GET /rest/ai_suggestions?profile_idx=N
 * Returns a suggested PID/threshold baseline derived from the saved
 * characterization model, plus the measured values it was derived from.
 * Purely advisory - nothing is written to the profile.
 */
